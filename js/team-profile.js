...
async function loadTeam() {
  const response = await fetch(`https://www.thesportsdb.com/api/v1/json/3/searchteams.php?t=${encodeURIComponent(countryName)}`);

  const data = await response.json();
  const team = data.teams?.[0];

  const playersResponse = await fetch(`https://www.thesportsdb.com/api/v1/json/3/lookup_all_players.php?id=${team.idTeam}`);
  const playersData = await playersResponse.json();

  const playersHtml = playersData.player ?.map(player => `
